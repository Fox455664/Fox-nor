...
async def botoot(client: Client, message: Message):
 if not client.me.username in oFFP:
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "Ux_Uv":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "رويال")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطور رويال للشات  💎 .\n♪ مرحبا بك : @Ux_Uv💎 .**")
    dev = await get_dev(bot_username)
    ...
    if message.new_chat_members[0].id == bot.id:
      ...
      button = [
[InlineKeyboardButton(text="✯ᥴꫝᥲ️ꪀꫀᏞ✯", url=f"{ch}"),InlineKeyboardButton(text="✯Ꮆ𝘳ꪮ??Ꮲ✯", url=f"{gr}")],
...
async def Ux_Uv(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("رويال",""))
...
@Client.on_message(
    filters.command(["/puma", "معلومات", "سورس", "السورس", "• السورس •"], "")
)
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton(user_id=f"royalbasha")
                        ],
                        [
...
@Client.on_message(filters.command(["المطور رويال","رويال","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="Ux_Uv")
...
@Client.on_message(filters.command(["المطور رويال"], ""))
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["Ux_Uv"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["Ux_Uv"]:
   ...
   if message.from_user.username == "Ux_Uv":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور رويال  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**JꪮᎥꪀ: ⦅ @U0_Ux⦆٠↝♬**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**JꪮᎥꪀ: ⦅ @U0_Ux⦆٠↝♬**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**JꪮᎥꪀ: ⦅ @U0_Ux⦆٠↝♬**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     bot_username = client.me.username
     dev = await get_dev(bot_username)
     if userr.username in ["Ux_Uv"] :
         await message.reply_text("**♪ رتبتك هي : مطور السورس  ✯ .**")
         return
     if userr.username in ["Ux_Uv"]:
         await message.reply_text("**♪ رتبتك هي : المطور رويال  ✯.**")
         return
     if userr.username in ["Ux_Uv"]:
         await message.reply_text("**♪ رتبتك هي : المطور رويال ✯.**")
         return
