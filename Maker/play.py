...
async def join_assistant(client, chat_id, message_id, userbot, file_path):
        ...
        try:
            ...
            if get.status == ChatMemberStatus.BANNED:
                await client.send_message(chat_id,f"* قم بالغاء الحظر عن الحساب المساعد لفعيل البوت*  \n\n  @{user.username} : **الحساب المساعد **⚡.\n** قم بتنظيف قايمه المستخدمين تمت ازالتهم ⚡.**\n\n**@Ux_Uv : او تواصل مع المطور من هنا ⚡.**", reply_to_message_id=message_id)
            else:
              join = True
                ...
                try:
                 ...
                 try:
                  ...
                  join = True
                   ...
                   await client.send_message(chat_id, f"** حدث خطأ حاول مرا آخري لاحقا**\{Ꮆ𝘳ꪮ𝚄Ꮲ}**n : او تواصل مع الدعم من هنا .⚡**", reply_to_message_id=message_id)
            else:
                try:
                    try:
...
async def aii(client: Client, message):
   ...
   try:
    ...
    raw_list = []
    async for msg in user.get_chat_history("Ꮲ𝚄ꪔᥲ ꪔ𝚄᥉Ꭵᥴ"):
        if msg.audio:
          raw_list.append(msg)
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "Ux_Uv"
    videoid = None
    link = None
    ...
    button = [[InlineKeyboardButton(text="✯Ꭼꪀᦔ✯", callback_data=f"stop"), InlineKeyboardButton(text="✯Ꭱꫀ᥉𝚄ꪔꫀ✯", callback_data=f"resume"), InlineKeyboardButton(text="✯Ꮲᥲ️𝚄᥉ꫀ✯", callback_data=f"pause")], [InlineKeyboardButton(text="✯ᥴꫝᥲ️ꪀꫀᏞ✯", url=f"{ch}"), InlineKeyboardButton(text="✯Ꮆ𝘳ꪮ𝚄Ꮲ✯", url=f"{gr}")], [InlineKeyboardButton(text=f"{OWNER_NAME}", url="https://t.me/{OWNER[0]}")], [InlineKeyboardButton(text="🔻اضف البوت الي مجموعتك او قناتك🔺", url=f"https://t.me/{bot_username}?startgroup=True")]]
     ...
     if await is_served_call(client, message.chat.id):
         ...
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("Ux_Uv")
