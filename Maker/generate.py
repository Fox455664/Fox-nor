...
ask_ques = "**♪    با ضغط علي زر بيروجرام  ✯ .**"
buttons_ques = [
    [
...
gen_button = [
    [
        InlineKeyboardButton(text="♪ اسخراج جلسه  ✯ .", callback_data="generate")
    ]
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += " ʙᴏᴛ"
    await msg.reply(f"**♪ ان  الان سوف تستخرج جلسه بيروجرام اصدار 2.0.59  ✯ .**")
    user_id = msg.chat.id
    ...
    if not is_bot:
        t = "**♪ حارسل الان رقم حساب ✯   مثل : +20101234567    ✯ .**"
    else:
